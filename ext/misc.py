...
class Misc(commands.Cog):
    ...
    async def gameawards(self, interaction: discord.Interaction):
        ...
        date = datetime(2026, 12, 11, 0, 30, 0, tzinfo=timezone.utc)
    ...
    @app_commands.describe(
        ...
        dice="How many dice you want to roll (max 100)")
    async def roll(self, interaction: discord.Interaction,
        sides: app_commands.Range[int, 2],
        dice: Optional[app_commands.Range[int, 1, 100]] = 1):
        """ Roll some dice! """
    ...
    async def yeahthatmakessense(self, message):
        if re.match(r"(?:yeah)[,.!]?\s(?:that makes sense)\W?$", message.content.lower()):
            if randint(1,4) == 1:
                await message.channel.send(
